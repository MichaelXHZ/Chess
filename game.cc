...
void Game::updateMoveBoard(int a,int b,char t,string s){
		...
		for (int j = 0; j < 8; ++j) {
			if (board[i][j] != nullptr) {
				...
				if (name == 'p') {
					...
					if (!board[i][j]->getIsMoved() && (board[i+2][j] == nullptr)) {
						// cout << "move 2" << endl;
	...
	info.second = "k1";
 	for(auto it = krange.begin(); it != krange.end(); ++it){
		...
		if (board[r][c] && (board[r][c]->getColour() == 'w')) moveBoard[r][c].emplace_back(info);
 	}
 	if ((kr == 0) && (kc == 4)) {
	...
	info.second = "K1";
 	...
 	for(auto it = Krange.begin(); it != Krange.end(); ++it){
		...
		if (board[r][c] && (board[r][c]->getColour() == 'b')) moveBoard[r][c].emplace_back(info);
 	}
 	if ((Kr == 7) && (Kc == 4)) {
